...
use Projeto\Entity\Projeto as Entity;
use \Cocur\Slugify\Slugify;

class Projeto extends EntityRepository {

    public function listagemMenuPrincipal() {

        $stmt = "SELECT q FROM $this->_entityName q Where "
                . "q.data_cancelamento IS NULL And q.status = 1 ORDER BY q.posicao ASC";

        return $this->_em->createQuery($stmt)->execute();
    }

    public function listagemComboContato() {

        $stmt = "SELECT q FROM $this->_entityName q Where "
                . "q.data_cancelamento IS NULL And q.status = 1 ORDER BY q.nome ASC";
