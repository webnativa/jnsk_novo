...
class Nucleo extends EntityRepository {
    
    public function getNucleoPorProjeto($projeto) {

        $stmt = "SELECT q FROM $this->_entityName q Where "
                . "q.data_cancelamento IS NULL And q.projeto = $projeto ORDER BY q.posicao ASC";
        
        return $this->_em->createQuery($stmt)->execute();
    }
    
    public function listagemFormContato() {
    ...
    public function listagemHome($projeto) {

        $stmt = "SELECT q FROM $this->_entityName q Where "
                . "q.data_cancelamento IS NULL And q.projeto = $projeto And q.status = 1 ORDER BY q.posicao ASC";
    ...
    public function save(array $data) {
        ...
        $projeto = $this->_em->getReference('\Projeto\Entity\Projeto', $data['projeto']);
